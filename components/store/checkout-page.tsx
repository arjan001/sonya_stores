  ...
  const handleWhatsAppCheckout = async () => {
    ...
    const message = encodeURIComponent(
      `Hi! I'd like to place an order:\n\n*ORDER DETAILS*\n${orderItems}\n\n*Subtotal:* ${formatPrice(totalPrice)}\n*Delivery:* ${
        ...
        window.open(`https://wa.me/254722123456?text=${message}`, "_blank")
    clearCart()
    setIsSubmitting(false)
                ...
                <div className="mt-6 space-y-3">
                  ...
                  {!isFormValid && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-xs text-red-700 space-y-1">
                      <p className="font-semibold">Complete your order information:</p>
                      <ul className="list-disc list-inside">
                        {!formData.name && <li>Full name is required</li>}
                        {!formData.phone && <li>Phone number is required</li>}
                        {!formData.address && <li>Delivery address is required</li>}
                        {!deliveryLocation && <li>Select a delivery option</li>}
                      </ul>
                    </div>
                  )}
